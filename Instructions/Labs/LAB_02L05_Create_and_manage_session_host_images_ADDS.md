...
1. Cloud Shell 창의 PowerShell 세션에서 다음을 실행하여 원본 이미지로 사용할 Windows 11 Enterprise 다중 세션을 실행하는 Azure VM을 배포합니다.
...
1. az140-25-vm0** 블레이드에서 커넥트** 선택하고 **드롭다운 메뉴에서 Bastion**을 통해 커넥트 선택합니다**.**
...
1. az140-25-vm0**에 대한 **Bastion 세션 내에서 관리istrator: Windows PowerShell ISE** 콘솔에서 **다음을 실행하여 이미지 구성을 위한 임시 위치로 사용할 폴더를 만듭니다.
   ...
   > **참고**: 클래식 Microsoft Teams의 설치 및 구성을 단계별로 진행합니다(이 랩에 사용된 이미지에 Teams가 이미 있으므로 학습 목적으로).

1. az140-25-vm0에 대한 **Bastion 세션 내에서 제어판 > 프로그램 > 프로그램 및 기능**으로 이동하고 **Teams Machine-Wide Installer** 프로그램을 마우스 오른쪽 단추로 클릭하고 **제거**를 선택합니다**.**
...
1. az140-25-vm0에 대한 **Bastion 세션 내에서 Microsoft Edge에서 VM[에 Teams 데스크톱 앱 배포라는 설명](https://learn.microsoft.com/en-us/microsoftteams/teams-for-vdi#deploy-the-teams-desktop-app-to-the-vm)서 페이지로 이동하고, 64비트 버전** 링크를 클릭한 **다음, 메시지가 표시되면 C:\\Allfiles\\Labs\\02** 폴더에 **Teams_windows_x64.msi** 파일을 저장**합니다.**
...
1. az140-25-vm0**에 대한 **Bastion 세션 내에서 Windows PowerShell ISE**를 관리istrator로 시작하고 **관리istrator: Windows PowerShell ISE** 콘솔에서 **다음을 실행하여 Microsoft Edge를 설치합니다(이 랩에 사용된 이미지에 Edge가 이미 있으므로 학습 목적으로).
   ...
   > **참고**: 다음으로 Windows 자동 업데이트 사용하지 않도록 설정하고, Storage Sense를 사용하지 않도록 설정하고, 표준 시간대 리디렉션을 구성하고, 원격 분석 컬렉션을 구성합니다. 일반적으로 가장 최근의 품질 업데이트를 먼저 적용해야 합니다. 이 랩에서는 랩의 기간을 최소화하기 위해 이 단계를 건너뜁니다.
...
1. **관리istrator: C:\windows\system32\cmd.exe** 창의 명령 프롬프트에서 디스크 정리 유틸리티를 실행하고 완료되면 확인을** 클릭합니다**.
   ...
   > **참고**: 디스크 클린업 프로세스는 3-5분 정도 걸릴 수 있습니다.
...
1. 랩 컴퓨터의 **커넥트ion 오류** 대화 상자에서 닫기를** 선택합니다**.
...
1. **이미지** 만들기 블레이드의 기본 사항 탭에서 **다음 설정을 지정합니다**.
   ...
   |대상 Azure Compute Gallery|az14025imagegallery라는 **새 갤러리 만들기**|
   |운영 체제 상태|**일반화**|
...
1. 이미지 만들기 블레이드의 **기본 사항** 탭으로 돌아가서 다음 설정을 지정하고 검토 + 만들기**를 클릭합니다**.** ** 
   ...
   |---|---|
   |버전 번호|**1.0.0**|
   |최신 항목에서 제외|검사box가 지워진 경우|
   |수명 주기 끝|현재 날짜보다 1년 앞당기기|
   |기본 복제본(replica) 개수|**1**|
...
1. **이미지** 만들기 블레이드의 검토 + 만들기** 탭에서 **만들기**를 클릭합니다**.
   ...
   > **참고**: 배포가 완료될 때까지 기다립니다. 이 작업은 약 10-15분 정도 걸릴 수 있습니다.
...
1. **호스트 풀** 만들기 블레이드의 **검토 + 만들기** 탭에서 만들기**를 선택합니다**.
   ...
   > **참고**: 할당량 제한에 도달하여 배포가 실패하는 경우 첫 번째 랩에서 철자가 지정된 단계를 수행하여 표준 D2sv3 제한의 할당량 증가를 30으로 자동으로 요청합니다.

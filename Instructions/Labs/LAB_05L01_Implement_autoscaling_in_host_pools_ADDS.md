...
# <a name="lab---implement-autoscaling-in-host-pools-ad-ds"></a>랩 - 호스트 풀에서 자동 크기 조정 구현(AD DS)
...
### <a name="exercise-1-configure-autoscaling-of-azure-virtual-desktop-session-hosts"></a>연습 1: Azure Virtual Desktop 세션 호스트의 자동 크기 조정 구성
...
#### <a name="task-1-prepare-for-autoscaling-of-azure-virtual-desktop-session-hosts"></a>작업 1: Azure Virtual Desktop 세션 호스트의 자동 크기 조정 준비
...
1. **az140-dc-vm11**에 연결된 원격 데스크톱 세션 내의 **관리자: Windows PowerShell ISE** 스크립트 창에서 다음 명령을 실행하여 자동 스케일링 솔루션의 일부분인 Azure Automation 계정을 만드는 데 사용할 PowerShell 스크립트를 다운로드합니다.
...
1. **az140-dc-vm11**에 연결된 원격 데스크톱 세션 내의 **관리자: Windows PowerShell ISE** 스크립트 창에서 스크립트 출력을 검토합니다. 
   ...
   >**참고**: 웹후크 URI의 값을 기록합니다. 이 랩의 후반부에서 필요합니다.
...
1. **az140-dc-vm11**에 연결된 원격 데스크톱 세션 내에서 **관리자: Windows PowerShell ISE** 창으로 전환한 다음, **관리자: Windows PowerShell ISE** 스크립트 창에서 다음 명령을 실행하여 자동 스케일링 솔루션의 일부분인 Azure 논리 앱을 만드는 데 사용할 PowerShell 스크립트를 다운로드합니다.
...
1. **az140-dc-vm11**에 연결된 원격 데스크톱 세션 내의 **관리자: Windows PowerShell ISE** 스크립트 창에서 다음을 실행하여 스크립트 매개 변수에 할당할 변수 값을 설정합니다(`<webhook_URI>` 자리 표시자를 이 랩의 앞부분에서 기록한 웹후크 URI 값으로 대체).
...
1. **az140-dc-vm11**에 연결된 원격 데스크톱 세션 내의 **관리자: Windows PowerShell ISE** 스크립트 창에서 다음 명령을 실행하여 자동 스케일링 솔루션의 일부분인 Azure 논리 앱을 만듭니다.
   ...
   >**참고**: 스크립트가 완료될 때까지 기다립니다. 2분 정도 걸릴 수 있습니다.
...
### <a name="exercise-2-verify-and-review-autoscaling-of-azure-virtual-desktop-session-hosts"></a>연습 2: Azure Virtual Desktop 세션 호스트의 자동 크기 조정 확인 및 검토
...
#### <a name="task-1-verify-autoscaling-of-azure-virtual-desktop-session-hosts"></a>작업 1: Azure Virtual Desktop 세션 호스트의 자동 크기 조정 확인
...
#### <a name="task-2-use-azure-log-analytics-to-track-azure-virtual-desktop-events"></a>작업 2: Azure Log Analytics를 사용하여 Azure Virtual Desktop 이벤트 추적
...
1. Log Analytics 작업 영역 블레이드 왼쪽의 세로 메뉴에 있는 **일반** 섹션에서 **로그**를 클릭하고 필요한 경우 **Log Analytics 시작** 창을 닫고 **쿼리** 창으로 이동합니다.
...
1. 쿼리 창에 다음 쿼리를 붙여넣고 **실행**을 클릭하여 이 랩에서 사용하는 호스트 풀의 모든 이벤트를 표시합니다.
   ...
   >**참고**: 잘라내기 및 붙여넣기 구문을 사용할 때 두 번째 줄에 추가 파이프 문자(|)가 있는 경우 이를 제거하여 실패를 방지합니다. 이는 각 쿼리에 적용될 수 있습니다.
